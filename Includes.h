...
public:
	...
	static inline std::string tostr(double n)
	{
		...
		if (n == (int)n) res2 = "";
		else while (n != 0) n *= 10, res2 += ((int)n + '0'), n = n - (int)n;
		return (op ? "" : "-") + res + res2;
	}
	...
	static inline void tgfill(int grid)
	{
		for (int i = 0; i < MAXN; i++)
		{
	...
	static inline void tgunfill(int grid)
	{
		for (int i = 0; i < MAXN; i++)
		{
			if (get_status(grid, i) != O) continue;
			change_status(grid, i, BLANK);
			change_status(i, grid, BLANK);
		}
	}
	...
	static inline std::pair<int, double> tgresearch(int depth, int abdfs_depth, int grid)
	{
		if (depth == 1)
		{
			return std::make_pair(-1, abdfs(abdfs_depth, true, grid).second);
		}
		int k = Moves::tiger_move(grid);
		change_status(grid, k, X);
		double best_heur = uninitialized; int best_move;
		tgfill(grid);
		for (int i = 0; i < MAXN; i++)
		{
			if (get_status(grid, i) != O) continue;
			change_status(i, grid, BANNED);
			auto res = tgresearch(depth - 1, abdfs_depth, i);
			double heur = res.second;
			if (heur < best_heur) best_move = i, best_heur = heur;
			change_status(i, grid, X);
		}
		tgunfill(grid);
		change_status(grid, k, BLANK);
		return std::make_pair(best_move, best_heur);
	}
	static inline std::pair<std::vector<int>, std::pair<int, double>> tgresearch_with_traceback(int depth, int abdfs_depth, int grid)
	{
		if (depth == 1)
			return std::make_pair(std::vector<int>(), std::make_pair(-1, abdfs(abdfs_depth, true, grid).second));
		std::vector<int> best_vec;
		int k = Moves::tiger_move(grid); change_status(grid, k, X);
		double best_heur = uninitialized; int best_move;
		tgfill(grid);
		for (int i = 0; i < MAXN; i++)
		{
			if (get_status(grid, i) != O) continue;
			change_status(i, grid, BANNED);
			auto res = tgresearch_with_traceback(depth - 1, abdfs_depth, i);
			double heur = res.second.second; std::vector<int> traceback = res.first; traceback.push_back(i);
			if (heur < best_heur) best_move = i, best_heur = heur, best_vec = traceback;
			change_status(i, grid, X);
		}
		tgunfill(grid);
		change_status(grid, k, BLANK);
		return std::make_pair(best_vec, std::make_pair(best_move, best_heur));
	}
	...
	static inline void get_playing_mode()
	{
		...
		if (version == CHI) std::cout << "你想要玩什么模式？请输入 CVSC, CVSP, PVSC, PVSP, TVSC, 或者 TVSP。P代表人, C代表机, T代表反身策略。\n";
		std::cin >> input;
		while (input != "CVSC" && input != "CVSP" && input != "PVSC" && input != "PVSP" && input != "TVSC" && input != "TVSP")
		{
			...
			if (version == CHI) std::cout << "输入错误！请再次输入 CVSC, CVSP, PVSC, PVSP, TVSC, 或者 TVSP。\n";
			std::cin >> input;
		}
		...
		else if (input == "TVSP") playing_mode = TVSP;
	}
	...
	public:
		...
		static inline std::pair<int, double> tvsp_move(bool is_x, int now_grid)
		{
			if (is_x) return std::make_pair(tiger_move(now_grid), uninitialized);
			else {
				tgfill(now_grid);
				int n;
				std::cin >> n; n--;
				change_status(now_grid, n, BLANK);
				change_status(n, now_grid, BLANK);
				return std::make_pair(n, uninitialized);
			}
		}
		static inline std::pair<int, double> auto_move(bool is_x, int now_grid)
		{
			...
			if (playing_mode == TVSP) return tvsp_move(is_x, now_grid);
		}
	};
};
